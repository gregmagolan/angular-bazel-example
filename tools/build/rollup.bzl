...
def _rollup_bundle_impl(ctx):
  build_directory = str(ctx.bin_dir)[:-len('[derived]')]
  ...
  args = ["--format", "es"]
  args += ["--input", '/'.join([build_directory, ctx.attr.entry_point])]
  ...
  print("rollup args", args)
  print("rollup bin_dir", ctx.bin_dir)
  ctx.action(
      progress_message = "Rollup bundling %s" % ctx.attr.entry_point,
